...
#### 판매 관리 3탭 (v0.43~0.45 / 임원용 v2.3.689~691)

| 탭 | 방향 | 문서 |
|---|---|---|
| 주문 관리 | 미러 + **허용 필드만 되돌림** | `orders` ↔ `inbox_orders` |
| 위탁 재고이동 | 미러(명세서용 상세 포함) | `consign` |
| 광고비 관리 | 미러 + 등록 요청 | `ads`·`adprojects` ↔ `inbox_ads`·`inbox_adprojects` |

⚠️ **주문은 레코드를 통째로 덮어쓰면 안 된다.** 직원용 미러에는 고객명·연락처·주소가
아예 없어서, `Object.assign` 하면 그 칸이 빈 값으로 날아간다(2026-08-16 사고와 같은 구조).
임원용 `_STAFF_ORDER_EDITABLE = ['status','stockStatus','qty','note']` 밖은 건드리지 않는다.
빈 문자열('변경 없음')도 건너뛴다.

**위탁 명세서** — 공급단가는 원가가 아니라 공급가라 직원에게 보인다(사용자 확인).
거래처용은 금액 포함, 3PL용은 **단가·공급가액·세액·합계를 모두 비운다** — 하나만 지우면
합계÷수량으로 복원되기 때문. 미러(`consign`)에 `lines`/`buyer`/`supplier`/`vatMode` 를 싣는다.

**위탁 출고 등록(v0.46.0)은 직원 업무다.** 직원이 직접 등록하되 **재고 계산은
임원용 `_applyStockMove` 를 그대로 쓴다** — 재고 규칙이 두 벌이 되면 갈라진다.
직원용은 `inbox_consign` 에 요청을 올리고, 임원용 `_staffInboxConsign()` 이 적용해
성공/실패(품목별 사유)를 되돌려준다. 기준정보(판매업체·공급단가)는 `consign_meta` 미러.

⚠️ **`cs-` 이름공간은 약사 CS 가 이미 쓰고 있다.** 위탁 출고는 `cg-` 로 분리했다
(`ovl-cg`·`cg-f-*`·`openCgModal`·`CGI`). 새 화면을 붙일 때 접두어 충돌을 먼저 확인할 것 —
`let` 중복 선언이면 스크립트 전체가 죽는다.

**광고비 카드** — 원가가 필요한 NET 마진·주문당 순익·손익분기 ROAS 는 넣지 않았다.
실 광고비·ROAS·CAC·AOV·전월 대비만 매출·주문수로 계산한다.
