...
- ⚠️ **이름 정확일치라, 서버 라벨이 한 글자만 달라도 칸이 조용히 빈다.**
  2026-08-16 에 르니브 창고가 그랬다. ERP 재고관리에는 1,760·3,286·5,629 가 있는데
  3D 는 `스냅샷에 이 창고가 없음` — 서버 `vo_snapshot.py` 의 옛 `WH_NAMES` 가 이 창고를
  **`르니브 본창고`** 로 라벨링하고 있었다. `WH_ALIAS` 에 `{스냅샷 이름: 칸 이름}` 을
  넣어 연결한다(`whCanon()` 이 적용). 서버를 못 고치는 상황에서도 바로 뜬다.
...
**수량은 ERP 재고관리 화면의 `재고 위치 / 재고 수량` 열과 같은 값**(`item.whStock`)이다.
`item.stock - 배정합계`(미배정 잔여)를 기본 창고에 더하지 말 것 — ERP 화면도 그 값을 창고 칸에
표시하지 않아서, 더하면 3D 숫자가 화면과 어긋난다. (2026-08-15 실제로 어긋나 되돌린 이력)
