...
| 창고 `warehouse` | -58, 0 | 18×58 | 창고 4칸 일렬 — 칸마다 창고명 + 재고 상자 |
...
const WH = {
  x: -58,
  bayZ:  [-19.5, -6.5, 6.5, 19.5],
  names: ['르니브 창고', '르니브 본사', '쿠팡 창고', '바로팜 창고'],
};
const WH_HIDDEN = ['바이오코스텍 창고', '내추럴솔루션 창고'];   // 일부러 뺀 곳
const WH_ALIAS  = { '르니브 본창고': '르니브 창고', '본창고': '르니브 창고' };
```

**2026-08-16: 6칸 → 4칸.** 바이오코스텍·내추럴솔루션은 외주처라 화면에서 뺐고,
남은 칸을 넓혀(간격 9.5 → 13, 선반 길이 8.2 → 10.8) 상자·간판이 잘 보이게 했다.
칸 수를 바꿀 때 **같이 손봐야 하는 값**: `bayZ` 개수, `SHELF_SLOT` 의 `dz`,
선반 기둥 위치(`[-5.2, 5.2]`), 선반판·백판 길이, 레일 길이(`whRail`), 간판 폭.
칸만 줄이고 나머지를 그대로 두면 선반이 옆 칸을 침범한다.

- **칸은 '자리'다.** `names` 순서로 4칸이 항상 존재하고, 데이터가 없어도 간판은 걸린다.
  스냅샷의 창고를 **이름으로 매칭**해 채운다(공백 무시). 순서에 의존하지 않는다.
- ⚠️ **이름 정확일치라, 서버 라벨이 한 글자만 달라도 칸이 조용히 빈다.**
  `WH_ALIAS` 에 `{스냅샷 이름: 칸 이름}` 을 넣어 연결한다(`whCanon()` 이 적용).
  지금 들어 있는 `르니브 본창고 → 르니브 창고` 는 **데모 스냅샷(`?demo=1`)용**이다.
  실서버는 v2 부터 ERP 이름을 그대로 쓰므로 별칭이 필요 없다.
  → 2026-08-16 에 르니브 창고가 빈 것은 **이름 문제가 아니라 계산식 문제**였다
  (아래 '수량' 항목의 정정 참고). 처음엔 이름 불일치로 짚었다가, 빈 칸 자가진단이
  스냅샷에 그 이름이 아예 없다는 걸 보여줘서 원인을 다시 찾았다.
- **빈 칸을 클릭하면 스냅샷이 실제로 담고 있는 창고 이름이 전부 나온다.** 콘솔에도
  `[창고] 스냅샷 창고: [이름(N종), ...]` 로 찍는다. 이름을 추측하지 말고 이걸 볼 것 —
  서버 라벨이 또 바뀌어도 같은 방법으로 바로 짚힌다.
- 매칭 안 된 창고는 화면에서 빠지고 콘솔에 `[창고] 칸에 매칭되지 않은 창고: [...]` 로 찍힌다.
  **이 경고가 보이면 `WH.names` 나 서버 `WH_NAMES` 중 하나가 틀린 것이다.**
  단 `WH_HIDDEN` 에 든 이름은 `일부러 제외한 창고` 로 따로 찍는다 — 의도한 제외와
  진짜 설정 실수를 콘솔에서 구분하려고 나눠 놓은 것이다.
...
- **간판과 명패 둘 다 클릭하면 그 창고의 재고 목록이 뜬다**(품목수·총수량·부족수 +
  품목별 수량 전부). 간판은 `renderData` 밖에서 한 번만 만들어지므로 `WH_SIGN_PICK[i]`
  로 참조를 들고 있다가 `userData.info` 를 갱신한다.
  → 2026-08-16 이전에는 간판에 안내문만 있었다. 눈에 띄는 건 간판인데 재고는 명패에만
  달려 있어서, 간판을 눌러 본 사람에겐 "재고가 안 뜬다"로 보였다.
  **클릭 가능한 오브젝트를 새로 만들 때 같은 실수를 조심할 것.**
- 선반에는 `SHELF_SLOT` 개수(6종)까지만 놓이지만, 클릭 목록은 전부 보여준다.
  잘린 경우 패널 아래에 몇 종까지 놓였는지 표시한다.
...
**수량은 ERP 재고관리 화면의 `재고 위치 / 재고 수량` 열과 같은 값**이어야 한다.
그 열은 `item.whStock`(명시 배정)이 아니라 **`_itemWhEffective()`** 를 쓴다 —
명시 배정 **+ 미배정 잔여(`item.stock - 배정합계`)를 기본 창고(`defaultWh`)에 귀속**.
기본 창고가 없으면 `미지정`으로 두고 어느 칸에도 넣지 않는다. (ERP `index.html` 29028행)

> ⚠️ **2026-08-16 정정.** 이 문서에는 오래도록 "미배정 잔여를 기본 창고에 더하지 말 것"
> 이라고 **반대로** 적혀 있었고, 서버 `vo_snapshot.py` 도 그렇게 만들어져 있었다.
> 그 결과 **르니브 창고가 스냅샷에서 통째로 빠졌다** — 이 창고에 걸린 1,760 / 3,286 /
> 5,629 는 전부 미배정 잔여였고 `whStock` 에는 키조차 없었기 때문이다.
> ERP 쪽 주석도 `르니브 창고는 파생값이라` 라고 적고 있다(29030행).
> 검산: 1,985 = 쿠팡 225 + 잔여 1,760 / 3,443 = 바로팜 157 + 잔여 3,286
> / 5,742 = 바로팜 113 + 잔여 5,629. 세 품목 모두 합계가 맞아떨어진다.

`창고 실제 재고` 열은 PlusCL(물류대행) 실시간 재고로 **다른 소스**이며 창고별 분해가 없다.

⚠️ **스냅샷에 `id` 가 있어야 이미지가 붙는다.** 서버
...
⚠️ **서버 `vo_snapshot.py` 의 창고 라벨이 ERP 이름과 다르다.**
옛 버전은 `WH_NAMES` 에 창고 ID 3개만 하드코딩했고(`르니브 본창고`·`바로팜 창고`·
`쿠팡 창고`), 그래서 ERP 의 **`르니브 창고`** 가 **`르니브 본창고`** 로 나간다.
지금은 클라이언트 `WH_ALIAS` 로 연결해 뒀다 — **화면은 정상이지만 근본은 서버에 있다.**
`world/vo_snapshot.py` 에 `shared/warehouse_list` 를 읽어 ERP 이름을 그대로 쓰는 v2 를
만들어 뒀다(하드코딩 제거). 서버에 설치하면 `WH_ALIAS` 는 지워도 된다.
설치 여부는 빈 칸 클릭이나 콘솔의 `[창고] 스냅샷 창고:` 로 확인할 수 있다.
...
- **GitHub Desktop이 저장소를 못 찾음** → 저장소가 `C:\dev\ERP` 로 옮겨졌다.
  기존 항목을 제거하고 `File → Add local repository` 로 이 경로를 다시 등록할 것.
