...
## 5-1. 색감 규칙 (2026-08-15b)

전체를 **밝은 파스텔 톤**으로 맞춰 뒀다. 색을 만질 때 지킬 것:

- 색은 `const P = {...}` 팔레트에 모아 두고, 개별 오브젝트에 새 hex를 흩뿌리지 말 것.
- **밝게 = 흰색을 섞는 게 아니라, 채도를 낮추고 명도를 올리는 것.** 전부 흰색에 가깝게 만들면
  창고처럼 물건이 바닥에 묻혀 안 보인다 (실제로 한 번 그렇게 됐다).
- 바닥·러그와 그 위에 놓이는 물건은 **색상(hue)으로 구분**한다. 명암 차이로만 구분하면 파스텔에서 무너진다.
  예) 크림색 바닥(`#F8F1E5`) 위의 재고 상자는 살구색(`#F0B876`).
- 조명 기준값: `AmbientLight 1.08` / `HemisphereLight 0.85` / `DirectionalLight 0.78`.
  환경광을 1.3 이상 올리면 입체감이 사라져 종이처럼 보인다.
